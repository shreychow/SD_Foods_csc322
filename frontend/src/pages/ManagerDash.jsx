...
//   const [stats, setStats] = useState({ orders: 0, revenue: 0, users: 0, feedback: 0 });
...
//   const handleApproveFeedback = async (id, isComplaint, targetId) => {
//     try {
...
//     try {
//       await client.post(`/manager/feedback/${id}/dismiss`, { reporter_id: fromId });
...
//   if (!manager || loading) return <div className="page-center"><p>Loading...</p></div>;

//   const pendingFeedback = feedback.filter((f) => f.status === "open" || f.status === "pending");
...
//             <h3 className="title-md mb-3">Pending Feedback Review</h3>
...
//                       <strong>From:</strong> {fb.from} → <strong>To:</strong> {fb.target}
//                     </p>
//                     <p className="text-muted" style={{ margin: "0 0 15px 0" }}>{fb.message}</p>
//                     <div className="flex gap-sm">
//                       <button className="btn btn-success btn-sm" onClick={() => handleApproveFeedback(fb.id, fb.type === "complaint", fb.targetId)}>
//                         <CheckCircle size={16} /> Approve
//                       </button>
//                       <button className="btn btn-danger btn-sm" onClick={() => handleDismissFeedback(fb.id, fb.fromId)}>
...
//             {employees.length === 0 ? (
//               <p className="text-muted text-center" style={{ padding: "40px" }}>No employees found</p>
//             ) : (
//               <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
//                 {employees.map((emp) => (
...
//                         </p>
//                         <p className="text-small text-muted">Warnings: {emp.amount_warnings || 0}</p>
//                       </div>
//                     </div>
//                     <div className="flex gap-sm mt-2">
//                       <button className="btn btn-success btn-sm" onClick={() => handlePromoteEmployee(emp.user_id, emp.role)}>
//                         <TrendingUp size={14} /> Promote
//                       </button>
//                       <button className="btn btn-secondary btn-sm" onClick={() => handleDemoteEmployee(emp.user_id, emp.role)}>
//                         <TrendingDown size={14} /> Demote
//                       </button>
//                       <button className="btn btn-danger btn-sm" onClick={() => handleFireEmployee(emp.user_id, emp.role)}>
//                         <UserX size={14} /> Fire
//                       </button>
//                     </div>
//                   </div>
...
//             {customers.length === 0 ? (
//               <p className="text-muted text-center" style={{ padding: "40px" }}>No customers found</p>
//             ) : (
//               <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
//                 {customers.map((cust) => {
//                   const warnings = cust.amount_warnings || 0;
//                   const isVIP = cust.is_vip || false;
//                   const needsAction = warnings >= (isVIP ? 2 : 3);

//                   return (
//                     <div key={cust.user_id} className="card card-compact" style={{ background: needsAction ? "rgba(239, 68, 68, 0.05)" : undefined }}>
//                       <div className="flex-between mb-2">
//                         <div>
//                           <div className="flex gap-sm mb-1">
//                             <h4 style={{ margin: 0 }}>{cust.name}</h4>
//                             {isVIP && <Award size={16} style={{ color: "#f97316" }} />}
//                           </div>
//                           <p className="text-small text-muted">{cust.email}</p>
//                         </div>
//                         <div style={{ textAlign: "right" }}>
//                           <p style={{ margin: "0 0 5px 0" }}>
//                             Balance: ${parseFloat(cust.total_balance || 0).toFixed(2)}
//                           </p>
//                           <p className="text-small" style={{ color: warnings >= 2 ? "#ef4444" : "#78716c" }}>
//                             <AlertTriangle size={14} style={{ verticalAlign: "middle" }} /> {warnings} Warning(s)
//                           </p>
//                         </div>
//                       </div>
//                       {needsAction && (
//                         <div className="alert alert-error mb-2">
//                           {isVIP ? "VIP with 2+ warnings - will be demoted" : "Customer with 3+ warnings - will be deregistered"}
//                         </div>
//                       )}
//                       <div className="flex gap-sm">
//                         <button className="btn btn-danger btn-sm" onClick={() => handleDeregisterCustomer(cust.user_id)}>
