...
// frontend/src/pages/ManagerDashboard.jsx - COMPLETE REDESIGN
import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import {
  Users,
    ...
    <div className="page">
      ...
      <div className="container" style={{ paddingTop: "120px" }}>
        ...
        {activeTab === "feedback" && (
          <div className="card card-sm">
            <h3 className="title-md mb-3">
              <MessageSquare size={20} /> Pending Complaints ({pendingComplaints.length})
            </h3>
                  ...
                  >
                    ...
                    <div className="flex gap-sm">
                      ...
                      <button
                        className="btn btn-danger btn-sm"
                        onClick={() =>
                          handleDismissFeedback(fb.feedback_id, fb.feedback_from)
                        }
                      >
        ...
        {activeTab === "bids" && (
          <div className="card card-sm">
            <h3 className="title-md mb-3">
              <Truck size={20} /> Pending Delivery Bids ({Object.keys(bidsByOrder).length} Orders)
            </h3>
            {Object.keys(bidsByOrder).length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                {Object.values(bidsByOrder).map((order) => {
                  const lowestBid = Math.min(...order.bids.map((b) => b.bid_amount));
                  ...
                  return (
                    ...
                    >
                      <h4 className="title-md mb-2">Order #{order.order_id}</h4>
                          ...
                          return (
                            ...
                            >
                              <div className="flex-between mb-2">
                                ...
                                <strong style={{ color: isLowest ? "#22c55e" : "#f97316" }}>
                              ...
                              <div className="flex gap-sm">
                                <button
                                  className="btn btn-success btn-sm"
                                  onClick={() => handleApproveBid(bid.bid_id, isLowest)}
                                >
                                  <CheckCircle size={16} /> Approve
                                </button>
                                <button
        ...
        {activeTab === "vip" && (
          <div>
            ...
            {vipRequests.length > 0 && (
              <div className="card card-sm mb-3">
                <h3 className="title-md mb-3">
                  <Crown size={20} style={{ color: "#f97316" }} /> Pending VIP Requests ({vipRequests.length})
                </h3>
                <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                  {vipRequests.map((req) => (
                    <div
                    ...
                    >
                      ...
                      <div style={{ margin: "10px 0" }}>
                        <p className="text-small">
                          <strong>Balance:</strong> ${req.total_balance.toFixed(2)}
                        </p>
                        <p className="text-small">
            ...
            <div className="card card-sm">
              <h3 className="title-md mb-3">
                <Star size={20} style={{ color: "#eab308" }} /> Current VIP Customers
              </h3>
                    ...
                    .map((cust) => (
                      ...
                      >
                        ...
                        {cust.amount_warnings >= 2 && (
                          ...
                          >
                            <AlertTriangle size={16} /> VIP with 2+ warnings - consider demotion
                          </div>
                        )}
                        <button
                          className="btn btn-secondary btn-sm"
            ...
            {vipRequests.length === 0 && customers.filter((c) => c.vip_status).length === 0 && (
              <div className="card card-sm">
                <div className="text-center" style={{ padding: "60px 20px" }}>
                  <Crown size={48} style={{ color: "#f97316", margin: "0 auto 15px" }} />
                  <h3 className="title-md mb-2">No VIP Activity</h3>
                  <p className="text-muted">No pending requests or current VIP customers</p>
                </div>
              </div>
            )}
          </div>
                ...
                {employees.map((emp) => (
                  ...
                  >
                    <div className="flex-between mb-2">
                      ...
                      <div style={{ textAlign: "right" }}>
                        <p className="menu-price" style={{ margin: "0 0 5px 0" }}>
                    ...
                    <div className="flex gap-sm mt-2">
                      <button
                        className="btn btn-success btn-sm"
                        onClick={() => handlePromoteEmployee(emp.user_id, emp.name)}
                      >
                        <TrendingUp size={14} /> +10%
                      </button>
                      <button
                        className="btn btn-secondary btn-sm"
                        onClick={() => handleDemoteEmployee(emp.user_id, emp.name)}
                      >
                        <TrendingDown size={14} /> -10%
                      </button>
                      <button
                        className="btn btn-danger btn-sm"
                        onClick={() => handleFireEmployee(emp.user_id, emp.name)}
                      >
                        <UserX size={14} /> Fire
                      </button>
                    </div>
