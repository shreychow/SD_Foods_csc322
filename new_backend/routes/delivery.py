...
# new_backend/routes/delivery.py
...
def get_delivery_orders():
    ...
    try:
        ...
        # or are already out for delivery
        query = """
        SELECT
            o.order_id,
            o.delivered_to,
...
def pickup_order(order_id):
    """Delivery person picks up order (mark as Out for Delivery)"""
    try:
        conn = get_db_connection()
        if not conn:
            return jsonify({"error": "Database connection failed"}), 500
...
def deliver_order(order_id):
    """Delivery person delivers order (mark as Delivered)"""
    try:
        conn = get_db_connection()
        if not conn:
            return jsonify({"error": "Database connection failed"}), 500
